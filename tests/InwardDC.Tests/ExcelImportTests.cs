...
public class ExcelImportTests
{
    ...
    [Fact]
    public async Task ImportDispatchesAsync_SkipsDuplicateSerialWithinFile_AndImportsTheRest()
    {
        var (app, customerCode) = await SeedMastersAsync();
        using (app)
        {
            ...
            var dcs = await app.Uow.DCs.GetByPeriodDetailedAsync(new DateTime(2026, 1, 1), new DateTime(2026, 1, 3));
            var dc = Assert.Single(dcs);
            Assert.Single(dc.Items);
        }
    }

    [Fact]
    public async Task ImportDispatchesAsync_ImportsValidRows_WhenOtherRowsFail()
    {
        var (app, customerCode) = await SeedMastersAsync();
        using (app)
        {
