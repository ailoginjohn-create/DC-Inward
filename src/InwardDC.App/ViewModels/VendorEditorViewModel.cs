...
public partial class VendorEditorViewModel : EditorViewModelBase
{
    private readonly IVendorService _vendors;
    private Guid? _id;

    public VendorEditorViewModel(ICurrentUserService currentUser, IVendorService vendors)
        : base(currentUser)
    {
        _vendors = vendors;
        Title = "Vendor";
    ...
    public async Task InitializeAsync(Guid? id)
    {
        ...
        if (id.HasValue)
        {
            var dto = await _vendors.GetByIdAsync(id.Value);
            if (dto is null)
            {
                ShowError(new NotFoundException("Vendor not found."));
                return;
            }
        ...
        {
            Code = await _vendors.GenerateCodeAsync();
        }
    }
    ...
    private async Task SaveAsync()
    {
        ...
        {
            var request = new VendorSaveRequest
            {
                Id = _id,
                Code = Code,
                Name = Name,
            ...
            var result = await _vendors.SaveAsync(request);
            if (result.Success)
            {
